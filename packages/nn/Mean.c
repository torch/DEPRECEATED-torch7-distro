...
static const void* nn_Mean_id = NULL;

static int nn_Mean_forward(lua_State *L)
{
  ...
  THTensor_mul(output, 1./((double)input->size[dimension]));
  THTensor_select(output, NULL, dimension, 0);

  return 1;
}

static int nn_Mean_backward(lua_State *L)
{
  THTensor *input = luaT_checkudata(L, 2, torch_Tensor_id);
  ...
  THTensor_mul(gradInput, 1./((double)input->size[dimension]));
...
static const struct luaL_Reg nn_Mean__ [] = {
  {"forward", nn_Mean_forward},
  {"backward", nn_Mean_backward},
  {NULL, NULL}
};

void nn_Mean_init(lua_State *L)
{
  torch_Tensor_id = luaT_checktypename2id(L, "torch.Tensor");
  nn_Mean_id = luaT_newmetatable(L, "nn.Mean", NULL, NULL, NULL, NULL);
  luaL_register(L, NULL, nn_Mean__);
  lua_pop(L, 1);
}
