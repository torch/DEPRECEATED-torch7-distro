...
static const void* nn_Max_id = NULL;

static int nn_Max_forward(lua_State *L)
{
  ...
  TH_TENSOR_DIM_APPLY3(double, output, double, input, double, indices, dimension,
                       int theIndex = 0;
                       double theMax = input_p[0];
                       for(i = 1; i < input_size; i++)
                       {
                         if(input_p[i*input_stride] > theMax)
                         {
                           theIndex = i;
                           theMax = input_p[i*input_stride];
                         }
                       }
...
static int nn_Max_backward(lua_State *L)
{
  THTensor *input = luaT_checkudata(L, 2, torch_Tensor_id);
...
static const struct luaL_Reg nn_Max__ [] = {
  {"forward", nn_Max_forward},
  {"backward", nn_Max_backward},
  {NULL, NULL}
};

void nn_Max_init(lua_State *L)
{
  torch_Tensor_id = luaT_checktypename2id(L, "torch.Tensor");
  nn_Max_id = luaT_newmetatable(L, "nn.Max", NULL, NULL, NULL, NULL);
  luaL_register(L, NULL, nn_Max__);
  lua_pop(L, 1);
}
