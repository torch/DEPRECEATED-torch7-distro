...
#define TH_GENERIC_FILE "generic/Sum.c"
#else

static int nn_(Sum_forward)(lua_State *L)
{
...
static int nn_(Sum_backward)(lua_State *L)
{
  THTensor *input = luaT_checkudata(L, 2, torch_(Tensor_id));
...
static const struct luaL_Reg nn_(Sum__) [] = {
  {"Sum_forward", nn_(Sum_forward)},
  {"Sum_backward", nn_(Sum_backward)},
  {NULL, NULL}
};

static void nn_(Sum_init)(lua_State *L)
{
  luaT_pushmetaclass(L, torch_(Tensor_id));
  luaT_registeratname(L, nn_(Sum__), "nn");
  lua_pop(L,1);
}
