...
static int nn_(SpatialConvolution_forward)(lua_State *L)
{
  ...
  /* do convolutions */
  THLab_(xcorr2Dmv)(output, 1.0, input, weight, dH, dW, "vx");
...
static int nn_(SpatialConvolution_backward)(lua_State *L)
{
  ...
  /* gradient to kernels */
  THLab_(xcorr2DRevger)(gradWeight, 1.0, input, gradOutput, dH, dW);

  /* gradient to input */
  THTensor *tweight = THTensor_(newTranspose)(weight,0,1);
  THLab_(xcorr2Dmv)(gradInput, 0.0, gradOutput, tweight, dH, dW, "fx");
