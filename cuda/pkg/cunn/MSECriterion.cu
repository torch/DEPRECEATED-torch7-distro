...
__global__ void cunn_MSECriterion_backward_kernel(float *gradInput, float *input, float *target, float norm, int nframe, int dim)
{
  int k = blockIdx.x;
...
static int cunn_MSECriterion_backward2(lua_State *L)
{
  THCudaTensor *input = (THCudaTensor*)luaT_checkudata(L, 2, torch_CudaTensor_id);
  THCudaTensor *target = (THCudaTensor*)luaT_checkudata(L, 3, torch_CudaTensor_id);
  int sizeAverage = luaT_getfieldcheckboolean(L, 1, "sizeAverage");
  ...
  cunn_MSECriterion_backward_kernel<<<blocks,threads>>>(THCudaTensor_data(gradInput),
							THCudaTensor_data(input),
							THCudaTensor_data(target),
							norm,
							1, size);
...
static const struct luaL_Reg cunn_MSECriterion__ [] = {
  ...
  {"MSECriterion_backward2", cunn_MSECriterion_backward2},
  {NULL, NULL}
};
