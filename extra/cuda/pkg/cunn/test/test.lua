...
function cunntest.SpatialConvolutionMap_forward()
   ...
   local input = torch.randn(from,inj,ini)
   local sconv = nn.SpatialConvolutionMap(nn.tables.random(from,to,4),ki,kj,si,sj)
   local groundtruth = sconv:forward(input)
   local a = torch.Timer()
   for i = 1,nloop do
      groundtruth = sconv:forward(input)
   ...
   input = input:cuda()
   local gconv = nn.SpatialConvolutionMap(nn.tables.random(from,to,4),ki,kj,si,sj):cuda()
   gconv.weight = sconv.weight:cuda()
   gconv.bias = sconv.bias:cuda()
   gconv.connTableRev:cuda()
   local rescuda = gconv:forward(input)
   a:reset()
   for i = 1,nloop do
      rescuda = gconv:forward(input)
