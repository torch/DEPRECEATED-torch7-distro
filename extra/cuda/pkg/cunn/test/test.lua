...
function cunntest.SpatialConvolutionMap_forward()
   ...
   times[title] = tm
    local input = torch.randn(from,inj,ini)
   local sconv = nn.SpatialConvolutionMap(nn.tables.random(from,to,from),ki,kj,si,sj)
   groundtruth = sconv:forward(input)
   local a = torch.Timer()
   for i = 1,nloop do
      groundtruth = sconv:forward(input)
   ...
   tm.cpu = a:time().real
   input = input:cuda()
   local gconv = nn.SpatialConvolutionMap(nn.tables.random(from,to,from),ki,kj,si,sj):cuda()
   gconv.weight = sconv.weight:cuda()
   gconv.bias = sconv.bias:cuda()
